...
if [ -z "$1" ]; then
  ...
  echo 'ex: ./apply.sh $DTAPI $DTPASS $TENANTURL'
  exit 1
fi
...
if [ -z "$3" ]; then
  echo "Usage: TENANTURL is required to configure the OneAgent Operator"
  echo "example: /> export TENANTURL='abc12345.dynatrace.live.com'"
  echo "            --for Managed-- /> export TENANTURL='host_url/e/environmentid' "
  echo 'ex: ./apply.sh $DTAPI $DTPASS $TENANTURL'
  exit 1
fi

sed -i 's/ENVIRONMENTID.live.dynatrace.com/'"$3"'/' cr.yaml
